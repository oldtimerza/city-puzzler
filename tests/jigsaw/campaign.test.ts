...
describe("tutorial campaign", () => {
  it("provides a uniquely solvable 6x6 progression", () => {
    expect(CAMPAIGN_LEVELS.map((level) => level.id)).toEqual(["pair", "apart", "triad", "square-one", "square-set", "chord"]);
    expect(CAMPAIGN_LEVELS.map((level) => level.boardSize)).toEqual([6, 6, 6, 6, 6, 6]);
    ...
    expect(CAMPAIGN_LEVELS.slice(-3).some((level) => level.clues.some((placement) => placement.service === "factory"))).toBe(true);
